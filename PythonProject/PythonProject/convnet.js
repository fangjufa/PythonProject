...
(function (global) {
    ...
    var return_v = false;
    var v_val = 0.0;
    var gaussRandom = function () {//生成符合正态分布的随机数
        if (return_v) {
            return_v = false;
            return v_val;
        }
        var u = 2 * Math.random() - 1;//Math.random返回一个在[0,1)区间的伪随机数。
    ...
    var randn = function (mu, std) { return mu + gaussRandom() * std; }//生成服从(mu,std*std)分布的随机数。

    // Array utilities
    var zeros = function (n) {//相当于numpy的np.zeros(n)
    ...
    var arrContains = function (arr, elt) {//To check if arr contains elt.
    ...
    // return max and min of a given non-empty array.And their index.
    ...
    var randperm = function (n) {//生成[0,n-1]之间的随机排列的n维数组
        var i = n,
            j = 0,
            temp;
        var array = [];
